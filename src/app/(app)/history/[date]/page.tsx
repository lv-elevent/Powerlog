...
export default function HistoryDetailPage() {
  ...
  return <>
    <div className="flex items-center justify-between pt-4 md:pt-8"><div className="flex items-center gap-1"><BackLink href="/history" /><div><h1 className="sr-only">{formatRecordDateLabel(date)}</h1><div className="eyebrow">阅读模式 · {formatRecordDateLabel(date)}</div></div></div><Link href="/history" className="secondary-button px-3"><CalendarDays size={18} />日历</Link></div>
    ...
    <div className="mt-6"><SectionHeader title="完整时间线" action={<Link href={"/review/" + date} className="text-sm font-semibold text-blue-600">编辑复盘</Link>} /><div className="app-card p-5"><Timeline items={timeline} /></div></div>
