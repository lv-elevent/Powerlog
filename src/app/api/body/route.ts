...
export async function POST(request: Request) {
  try {
    ...
    const record = await upsertBodyMeasurement({ record_date: parsed.data.date, measured_at: parsed.data.measuredAt ?? new Date().toISOString(), weight_kg: parsed.data.weightKg, waist_cm: parsed.data.waistCm, note: parsed.data.note });
