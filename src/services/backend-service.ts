...
export function formatRecordDateLabel(date = currentRecordDate()): string {
...
export async function addWaterLog(input: { date: string; amountMl: number }): Promise<WaterLogRow | null> {
  const clientIdempotencyKey = crypto.randomUUID();
  return postWithOfflineFallback<WaterLogRow>("/api/water", { ...input, clientIdempotencyKey }, clientIdempotencyKey);
}

export async function saveBodyMeasurement(input: { date: string; weightKg: number; measuredAt?: string }): Promise<BodyMeasurementRow | null> {
  const clientIdempotencyKey = crypto.randomUUID();
  return postWithOfflineFallback<BodyMeasurementRow>("/api/body", { ...input, clientIdempotencyKey }, clientIdempotencyKey);
}

export async function addExpense(input: { date: string; amount: number; categoryId?: string | null; categoryName?: string | null; note?: string | null }): Promise<ExpenseRow | null> {
  const clientIdempotencyKey = crypto.randomUUID();
  return postWithOfflineFallback<ExpenseRow>("/api/expenses", { ...input, clientIdempotencyKey }, clientIdempotencyKey);
}

export async function addDailyNote(input: { date: string; text: string }): Promise<DailyNoteRow | null> {
  const clientIdempotencyKey = crypto.randomUUID();
  return postWithOfflineFallback<DailyNoteRow>("/api/notes", { ...input, clientIdempotencyKey }, clientIdempotencyKey);
}
...
export async function addWork(input: { date: string; sessionType: "work" | "study" | "project" | "other"; title: string; durationMinutes: number; didText?: string }): Promise<WorkSessionRow | null> { const clientIdempotencyKey = crypto.randomUUID(); return postWithOfflineFallback<WorkSessionRow>("/api/work", { ...input, clientIdempotencyKey }, clientIdempotencyKey); }
